...
def area_view(request, slug):
    ...
    return render(request, "web/area.html", {
        ...
        # `area_tickets`, not `tickets`: the Inbox page's own list (now Slices,
        # via _inbox_list.html/_inbox_row.html) uses `slices`. Reusing a bare
        # `tickets`/`slices` name here would quietly render the wrong list the
        # day the two meet on one page.
